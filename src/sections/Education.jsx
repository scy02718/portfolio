            ...
            <div className='mt-4 space-y-3 font-mono text-xs'>
                {educations.map(({ id, provider, degree, duration, gpa, transcript, skills, overview }) => (
                    <div key={id} className='border border-neon/30 bg-black/60 rounded-md p-3 shadow-neon-sm hover:border-neon-bright/50 hover:shadow-neon transition-all'>
                        ...
                        <p className='pl-4 mt-1 text-neon-glow/80'>{degree}</p>
                        <p className='pl-4 mt-1 text-neon-glow/70'>
                            <span className='text-neon/60'>↳ </span>{gpa}
                        </p>
                        <p className='pl-4 mt-2 text-neon-glow/70 leading-relaxed'>{overview}</p>
                        {skills?.length > 0 && (
                            <div className='pl-4 mt-2 flex flex-wrap gap-1.5'>
                                {skills.map((skill, i) => (
                                    <span key={i} className='px-1.5 py-0.5 border border-neon/30 rounded text-neon-glow/80'>
                                        {skill}
                                    </span>
                                ))}
                            </div>
